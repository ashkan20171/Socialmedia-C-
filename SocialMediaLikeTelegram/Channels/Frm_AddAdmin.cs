...
namespace SocialMediaLikeTelegram.Channels
{
    public partial class Frm_AddAdmin : Form
    {
        ...
        List<tbl_Users> NewAdminList;
        int TokenEditorLastHeight = 0;
        #region FormShadow
        ...
        {
            InitializeComponent();
            NewAdminList = new List<tbl_Users>();
        }
        ...
        private void Frm_AddAdmin_Load(object sender, EventArgs e)
        {
            ...
            {
                var AllMember = Rep_Channels.GetChannelMember(Chat_id);
                foreach (var item in Rep_Channels.GetChannelAdminList(Chat_id))
                    AllMember.Remove(item.tbl_Users);
            ...
            {
                var AllMember = Rep_Groups.GetGroupMember(Chat_id);
        ...
        private void ContactListItemClidked(object sender, EventArgs e)
        {
            ...
            if (Contact.IsActive)
            {
                ...
                NewAdminList.Remove(Rep_Users.GetUser(Contact.User_id));
            ...
            else
            {
                ...
                NewAdminList.Add(Rep_Users.GetUser(Contact.User_id));
        ...
        private void materialFlatButton1_Click(object sender, EventArgs e)
        {
            if (ChatType == Components.ComponentsProperties.ChatType.Channel)
            {
                if (NewAdminList.Count > 0)
                    if (AdvanceMethod.AdvanceMessageBox("Are you sure to Promote this Members to admin ?") == DialogResult.OK)
                    {
                        foreach (var item in NewAdminList)
                            Rep_Channels.AddChannelAdmin(item.id, Chat_id);
                        this.Close();
                    }
            }
            else
            {
                if (NewAdminList.Count > 0)
                    ...
                    {
                        foreach (var item in NewAdminList)
                            Rep_Groups.AddGroupAdmin(item.id, Chat_id, Frm_Chats.User_id);
                        this.Close();
                    }
            }
        }
