...
namespace SocialMediaLikeTelegram.Channels
{
    public partial class Frm_AddMember : Form
    {
        ...
        private void Frm_AddMember_Load(object sender, EventArgs e)
        {
            ...
            {
                foreach (var item in Rep_Channels.AddMemberChannelList(Chat_id,Frm_Chats.User_id))
                {
            ...
            {
                foreach (var item in Rep_Groups.AddMemberGroupList(Chat_id,Frm_Chats.User_id))
                {
                    Components.ContactListItem _contactlistitem = new Components.ContactListItem();
                    _contactlistitem.Top = AdminListItemTop;
                    _contactlistitem.User_id = item.id;
        ...
        private void materialFlatButton1_Click(object sender, EventArgs e)
        {
            if (ChatType == Components.ComponentsProperties.ChatType.Channel)
            {
                if (NewMemberList.Count > 0)
                    if (AdvanceMethod.AdvanceMessageBox("Are you sure to add this Contact's to channel ?") == DialogResult.OK)
                    {
                        foreach (var item in NewMemberList)
                            Rep_Channels.AddChannelMember(item.id, Chat_id);
                        this.Close();
                    }
            }
            else
            {
                if (NewMemberList.Count > 0)
                    if (AdvanceMethod.AdvanceMessageBox("Are you sure to add this Contact's to group ?") == DialogResult.OK)
                    {
                        foreach (var item in NewMemberList)
                            Rep_Groups.AddGroupMember(item.id, Chat_id);
                        this.Close();
                    }
            }
        }
