...
{
    public partial class Frm_BlockUser : Form
    {
        ...
        {
            var Q = Rep_Users.GetUserContactList(Frm_Chats.User_id);
        ...
        private void materialFlatButton1_Click(object sender, EventArgs e)
        {
            if (NewMemberList.Count > 0)
                if (AdvanceMethod.AdvanceMessageBox("Are you sure to block this users ?") == DialogResult.OK)
                {
                    foreach (var item in NewMemberList)
                        Rep_Users.BlockUser(item.id);
                    this.Close();
                }
        }
    }
