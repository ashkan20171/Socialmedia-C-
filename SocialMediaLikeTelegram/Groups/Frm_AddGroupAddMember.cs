...
{
    public partial class Frm_AddGroupAddMember : Form
    {
        ...
        {
            long Group_id = Rep_Groups.CreateGroup(Frm_AddGroupName.Group_name, Frm_AddGroupName.Des, Frm_AddGroupName.ProfilePic);
            Rep_Groups.AddGroupAdmin(Frm_Chats.User_id, Group_id, null);
            foreach (var item in NewMemberList)
                Rep_Groups.AddGroupMember(item.id, Group_id);
            this.Close();
        }
    }
}
