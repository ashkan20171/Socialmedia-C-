...
{
    public partial class Frm_AddBanUsers : Form
    {
        ...
        {
            foreach (var item in Rep_Groups.AvalableUsersToBanList(Group_id))
            {
        ...
        private void materialFlatButton1_Click(object sender, EventArgs e)
        {
            if (NewMemberList.Count > 0)
                if (AdvanceMethod.AdvanceMessageBox("Are you sure to ban this members's from group ?") == DialogResult.OK)
                {
                    foreach (var item in NewMemberList)
                    {
                        Rep_Groups.BanGroupUser(Group_id, item.id);
                        Rep_Groups.DeleteGroupMember(Group_id, item.id);
                        Form GroupInfo = (this.Owner.Owner);
                        Control c = GroupInfo.Controls["panel2"].Controls["PanelMembers"].Controls["MemberListItem_" + item.id.ToString()];
                        (this.Owner.Owner).Controls["panel2"].Controls["PanelMembers"].Controls.Remove(c);
                    }
                    this.Close();
                }
        }
