...
namespace SocialMediaLikeTelegram
{
    public partial class Frm_ContactList : Form
    {
        ...
        private int ContactListItemTop
        {
            get
            {
        ...
        private void Frm_ContactList_Load(object sender, EventArgs e)
        {
            foreach (var item in Rep_Users.GetUserContactList(Frm_Chats.User_id))
            {
                ...
                {
                    User_id = item.id,
                    ContactName = Rep_Users.GetContactName(Frm_Chats.User_id,item.id),
                    ProfilePic = item.tbl_Pic.Pic_Url,
                    LastSeen = Rep_PrivateChat.UserLastSeen(item.id),
                    Top = ContactListItemTop
                };
                _contactlistitem.Click += ContactClicked;
                panel2.Controls.Add(_contactlistitem);
        ...
        private void ContactClicked(object sender,EventArgs e)
        {
            ...
            Profiles.Frm_UserInfo.User_id = Contact.User_id;
            new Profiles.Frm_UserInfo().ShowDialog(this);
            foreach (Control item in panel2.Controls)
                panel2.Controls.Remove(item);
            if (panel2.Controls.Count > 0)
                panel2.Controls.RemoveAt(0);
            Frm_ContactList_Load(null, null);
        }
        private void materialFlatButton1_Click(object sender, EventArgs e)
        {
            new Contacts.Frm_AddContact().ShowDialog(this);
            foreach (Control item in panel2.Controls)
                panel2.Controls.Remove(item);
            if (panel2.Controls.Count > 0)
                panel2.Controls.RemoveAt(0);
